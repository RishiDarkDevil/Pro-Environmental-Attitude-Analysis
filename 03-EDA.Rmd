...
```{r corr_dendo_only, fig.asp=0.618, fig.width=14}
...
heatmap.2(mat, col = bluered(100), 
          ...
          main = "Jaccard Distance Based Clustering of Items and Students"
)
...
A2Rplot(
  ...
  col.down = c("#ff9900", "#4ECDC4", "#556270", "#ff66ff", "#00cc00", "#cc0000", "#cccc00", "#ADFF2F", "#000080"),
  show.labels=T,
  main="Jaccard Distance Based Clustered Items Dendrogram"
)
```
...
heatmap_dendo_plot <- function(qsn_dist = "euclidean", per_dist = "euclidean") {
  ...
  heatmap.2(mat, col = bluered(100), 
          ...
          main = "Maximum & L2 Distance Based Clustering of Domains and Students"
  )
}
...
-   Firstly, we see on the Domain Level PCA. The loading contribution of the first principal component has more weight on Strongly Disagree, Strongly Agree and Neutral with lesser but still comparable weight on Disagree. As expected the directions of Disagree, Neutral and Strongly Disagree are almost the same i.e. in the direction of PC, RES, SS, ES and hence they are correlated. The direction of Strongly Agree is around REC, SA, ER, CON. Whereas the second principal component puts maximum weight on Disagree and Agree in the directions of REC and little bit of weight on Disagree. We must understand that these are the directions around which the data varies the most. So, Strongly Agree doesn't vary much in all the domains except SA and same goes for the the other responses. The mean Strongly Agree is high and hence, overall we see large number of consistent Strongly Agrees with less variations and consistently large score of Agrees in all the directions.
